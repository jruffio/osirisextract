...
def _process_pixels(row_indices,col_indices,psfs_func_list, padcenter,hr8799_spec,hr8799c_spec, dtype):#,scaled_imgs_np,output_maps_np):
    ...
    for m,n in zip(row_indices,col_indices):
        ...
        if 0:
            ...
            print(output_maps_np[:,m,n])
        else:
            poldeg=2
            nl,nl,ny,nx = data.shape
            print("haha",nl,nl,ny,nx)
            ...
            for m in np.arange(0,ny):
                ...
                for n in np.arange(0,nx):
                    for k in np.arange(0,nl):
                        datavec = data[k,:,m,n]
                        modelvec = template[k,:,m,n]
                        wherefinite = np.where(np.isfinite(datavec))
                        # print(np.size(wherefinite[0]))
                        if np.size(wherefinite[0]) > 10:
                            x_samples = np.arange(np.size(wherefinite[0]))[:,None]
                            mat4poly = np.concatenate([x_samples**myexp for myexp in range(poldeg+1)],axis=1)
                            alldata.append(datavec[wherefinite])
                            allmodel.append(modelvec[wherefinite])
                            allmat.append(mat4poly)
            import time
            sp_allmat = block_diag(*allmat)
            print("ouh",sp_allmat.shape,np.concatenate(allmodel).shape)
            sp_allmat[1::,0] = np.concatenate(allmodel)
            print(np.linalg.solve(sp_allmat,np.concatenate(alldata)))
            time.sleep(1)
            exit()
            ...
            x_samples = np.arange(np.size(template))[:,None]
            Mat = np.concatenate([template[:,m,n],x_samples**0,x_samples**1,x_samples**2],axis=1)

            theta = np.linalg.solve(Mat,data)
        Npixproc+=1
...
if __name__ == "__main__":
    ...
    nl,ny,nx = imgs.shape
    psfs = psfs[0:100,:,:]
    hr8799c_spec = hr8799c_spec[0:100]
    hr8799_spec = hr8799_spec[0:100]
    wvs = wvs[0:100]
    ...
    tpool = mp.Pool(processes=1, initializer=_tpool_init,
    ...
    if 1:
        ...
        scaled_imgs_np = _arraytonumpy(scaled_imgs, scaled_imgs_shape,dtype=dtype)
        output_maps_np = _arraytonumpy(output_maps, output_maps_shape,dtype=dtype)
        nl,ny,nx = original_imgs_shape
        # nl,ny,nx=1665,74,29

        x_vec,y_vec = np.arange(nx * 1.)-padcenter[0],np.arange(ny* 1.)-padcenter[1]
        x_grid, y_grid = np.meshgrid(x_vec,y_vec)
        for m,n in zip([32],[11]):
            # print(m,n)
            # print(x_grid[m,n],y_grid[m,n]) #46.79802955665025 0.0
            template = np.zeros((nl,nl,3,3))
            for k,lambref in enumerate(wvs):
            ...
            for m in np.arange(0,ny):
                ...
                for n in np.arange(0,nx):
                    for k in np.arange(0,nl):
                        ...
                        if np.size(wherefinite[0]) > 10:
                            x_samples = np.arange(np.size(wherefinite[0]))[:,None]
                            print("poly_coefs",np.polyfit(x_samples[:,0],datavec[wherefinite]-(-5.25055644e-04)*modelvec[wherefinite],2)) #[-5.25055644e-04,  2.20857130e-01, -4.13931032e-04, ...,
       #  1.07829732e-01,  5.87592986e-04, -5.12051329e-06]), array([54.68887727]), 2701, array([4.19802001e+04, 4.19800739e+04, 4.19800739e+04, ...,
       # 3.22963452e+00, 3.22958469e+00, 2.08850852e+00]))
                            mat4poly = np.concatenate([x_samples**myexp for myexp in range(poldeg+1)],axis=1)
                            alldata.append(datavec[wherefinite])
                            allmodel.append(modelvec[wherefinite])
                            allmat.append(mat4poly)
            ...
            time.sleep(5)
            print("ouh",sp_allmat.shape,np.concatenate(allmodel).shape)
            sp_allmat = sp_allmat[1::,:]
            sp_allmat[:,0] = np.concatenate(allmodel)
            print(np.linalg.lstsq(sp_allmat,np.concatenate(alldata)))
            exit()

    # tpool.apply_async(_process_pixels, args=([32],[11],psfs_func_list, padcenter,hr8799_spec,hr8799c_spec, dtype))
    # for row_index, proc_pixs_task in enumerate(tasks):
    #     print("Finished image chunk {0}".format(row_index))
    #     proc_pixs_task.wait()
    # import time
    # time.sleep(10)
    #[-2.9874194e-04  6.9921487e-04 -4.2725340e-01]
    exit()
