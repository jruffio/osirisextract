...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        planet = "b"
        fit_folder = os.path.join(osiris_data_dir,"low_res","HR_8799_"+planet,"photo_mags_fit")
    ...
    if len(glob.glob(outgridname)) == 0:
        ...
        for file_id,grid_filename in enumerate(grid_filelist):
            ...
            model = []
            for wv,filt,xerr in zip(absmags_wvs,filters,xerrs.T):
    ...
    for Tid,T in enumerate(fitT_list):
        for loggid,logg in enumerate(fitlogg_list):
            for paraid,para in enumerate(fitpara_list):
                ...
                # model = myinterpgrid([1100,3.5,0.55])[0]

                wherefinite = np.where(np.isfinite(model))

                # plt.scatter(absmags_wvs,)
                # plt.show()


                _model = -2.5*np.log10(model[wherefinite]/vega_fluxes[wherefinite])
                _data = absmags[wherefinite]
                _variances = (absmags_err[wherefinite])**2

                ampl[Tid,loggid,paraid] = np.sum((_data-_model)/_variances)/np.sum(1/_variances)
                ...
                print(absmags_wvs[wherefinite])
                print(np.array(filters)[wherefinite])
                print(refs[wherefinite])
                plt.scatter(absmags_wvs[wherefinite],_data,label="data")
                plt.scatter(absmags_wvs[wherefinite],_model+ampl[Tid,loggid,paraid],label="model")
                plt.legend()
                plt.show()

                chi2 = np.sum(((_data-_model)-ampl[Tid,loggid,paraid])**2/_variances)
                # print(chi2)
    ...
    myoutfilename = planet+"_"+os.path.basename(gridname)+"_fit_photo_mags.pdf"
    try:
    ...
    plt.scatter(model_wvs[wherefinite],model[wherefinite]*10**(ampl[myargmax[0],myargmax[1],myargmax[2]]/-2.5),label="model",color="black")

    fluxes = vega_fluxes[wherefinite]*10**(absmags[wherefinite]/-2.5)
    yerr = [fluxes - vega_fluxes[wherefinite]*10**((absmags[wherefinite]+absmags_err[wherefinite])/-2.5),
            vega_fluxes[wherefinite]*10**((absmags[wherefinite]-absmags_err[wherefinite])/-2.5)-fluxes]
    print(yerr)
    plt.errorbar(absmags_wvs[wherefinite], fluxes,yerr = yerr, xerr=xerrs[:,wherefinite[0]], fmt="x", color = '#ff3300', capsize=5,
        elinewidth=1, markeredgewidth=1, label = "data")
    plt.ylim([0,1e-14])
    plt.legend()
    # plt.show()

    # x = np.tile(model_wvs[wherefinite][:,None],(1,2))
    # y
    # plt.plot(x,model[wherefinite]*ampl[myargmax[0],myargmax[1],myargmax[2]],label="model",color="black")
    # plt.show()


    if 1:
        print("Saving "+os.path.join(out_pngs,planet,planet+"_"+os.path.basename(gridname)+"_fit_photo_mags.pdf"))
        plt.savefig(os.path.join(out_pngs,planet,planet+"_"+os.path.basename(gridname)+"_fit_photo_mags.pdf"),bbox_inches='tight')
        plt.savefig(os.path.join(out_pngs,planet,planet+"_"+os.path.basename(gridname)+"_fit_photo_mags.png"),bbox_inches='tight')
    ...
    if 1:
        print("Saving "+os.path.join(out_pngs,planet,planet+"_"+os.path.basename(gridname)+"_corner_photo_mags.pdf"))
        plt.savefig(os.path.join(out_pngs,planet,planet+"_"+os.path.basename(gridname)+"_corner_photo_mags.pdf"),bbox_inches='tight')
        plt.savefig(os.path.join(out_pngs,planet,planet+"_"+os.path.basename(gridname)+"_corner_photo_mags.png"),bbox_inches='tight')
    plt.show()
