...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        outputdir = "/data/osiris_data/"+planet+"/20"+date+"/reduced_jb/20210616_new/"
    ...
    for res_it in range(2):
        ...
        if "kap_And" in filelist[0]:
            ...
            star_name = "kap_And"
            RV4fakes = -13.9
        if "GJ_504_b" in filelist[0]:
            phoenix_model_host_filename = glob.glob(os.path.join(phoenix_folder,"GJ_504_"+"*.fits"))[0]
            travis_spec_filename=os.path.join(planet_template_folder,
                                          "51Eri_b_highres_template.save")
            if IFSfilter == "Jbb":
                host_mag = 4.39
