...
def _process_pixels_onlyHPF(curr_k_indices,curr_l_indices,row_indices,col_indices,
    ...
    for curr_k,curr_l,row,col in zip(curr_k_indices,curr_l_indices,row_indices,col_indices):
        ...
        nospec_planet_model = nospec_planet_model+(np.exp(-0.5*(x_data_grid**2+y_data_grid**2)/1**2))[:,:,None]
        # nospec_planet_model[w-1:w+2,w-1:w+2,:] = 0.1
        # nospec_planet_model[w,w,:] = 1
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        ...
        # outputdir = "/data/osiris_data/"+planet+"/20"+date+"/reduced_jb/20200914_ql_combined/"
        ...
        filelist = glob.glob(os.path.join(inputDir,"s"+date+"*002*"+IFSfilter+"_"+scale+".fits"))
        filelist.sort()
        # filelist = [filelist[7],]
        # filelist = [filelist[0],]
        print(filelist)
    ...
    for res_it in range(2):
        ...
        for filename in filelist:
            ...
            ##############################
            # print(filename.replace("002_kbb","00{0}_kbb".format(0+2)))
            # # exit()
            # imgs_list = []
            # # imgs_hdrbadpix_list = []
            # for imit in range(5):
            #     with pyfits.open(filename.replace("002_kbb","00{0}_kbb".format(imit+2))) as hdulist:
            ...
            # imgs = np.nanmean(np.array(imgs_list),axis=0)
            # print(imgs.shape)
            # # exit()
