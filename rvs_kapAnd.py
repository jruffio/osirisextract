...
myoutfilename = "RV_kap_And_measurements.pdf"
# numbasis = 3
# for nbid,numbasis in enumerate([5,1,0]):
kl_list = [0,1,5,10]
for nbid,numbasis in enumerate(kl_list):
    if numbasis == 0:
        mylabel = r"RV (No PCA)"#"No PCA"
        mymark = "x"
        mycolor = "#ff9900"
    elif numbasis ==1:
        mylabel = r"RV ({0} PCs)".format(numbasis)
        mymark = "o"
        mycolor = "#0099cc"
    elif numbasis ==5:
        mylabel = "RV ({0} PCs)".format(numbasis)
        mymark = "*"
        mycolor = "#6600ff"
    elif numbasis ==10:
        mylabel = r"RV ({0} PCs)".format(numbasis)
    ...
    if numbasis ==0:
        c_fileinfos_filename = "/data/osiris_data/kap_And/fileinfos_Kbb_jb.csv"
    else:
        c_fileinfos_filename = "/data/osiris_data/kap_And/fileinfos_Kbb_jb_kl{0}.csv".format(numbasis)
    ...
    if 1:
        ...
        if 0:
            ...
            c_rvoffsets_list = -13.9-c_rvfakes_list
        ...
        # where_all_c = (where_all_c[0][::2],)
        where_esti_c = np.where(np.isfinite(c_good_rv_list))
        # where_esti_c = (where_esti_c[0][::2],)
        ...
        if 0: #print estimates per night
            c_combined_avg_list = []
            c_combined_sig_list = []
            c_chi2_list = []
            c_N_data_list = []
            c_uniquedate_list = []
            c_yr_list_new = [day[0:4] for day in c_date_list_new]
            ...
            for k,day in enumerate(unique_date):
                ...
                # exit()
                where_c = np.where(np.isfinite(c_good_rv_list)*(day == np.array(c_yr_list_new)))
            ...
            print(numbasis)
            print(unique_date)
            print(c_combined_avg_list)
            print(c_combined_sig_list)
            continue
        # exit()
        ...
        where_c_Kbb = np.where(np.isfinite(c_good_rv_list[where_all_c])*(np.abs(c_rel_res[where_all_c])<10))
        ...
        where_c_Kbbbad = np.where((np.abs(c_rel_res[where_all_c])>=10))
        ...
        plt.errorbar(c_indices[where_c_Kbb]+nbid/7.5,c_good_rv_list[where_all_c][where_c_Kbb]-c_bary_star_list[where_all_c][where_c_Kbb],
        ...
        plt.plot(c_indices[where_c_Kbb]+nbid/7.5,c_good_rv_list[where_all_c][where_c_Kbb]-c_bary_star_list[where_all_c][where_c_Kbb],mymark,color=mycolor,label=mylabel)
        ...
        # plt.plot([0,np.size(where_all_c[0])],[c_combined_avg+c_combined_sig,c_combined_avg+c_combined_sig],linestyle="--",alpha=0.4,color=mycolor)
        # plt.plot([0,np.size(where_all_c[0])],[c_combined_avg-c_combined_sig,c_combined_avg-c_combined_sig],linestyle="--",alpha=0.4,color=mycolor)
        # plt.fill_between([0,np.size(where_all_c[0])],c_combined_avg-c_combined_sig,c_combined_avg+c_combined_sig,alpha=0.2,color=mycolor)

        c_chi2 = np.nansum(((c_good_rv_list[where_c_Kbb]-c_bary_star_list[where_c_Kbb]-c_combined_avg)/np.sqrt(c_rvsig_list[where_c_Kbb]**2+c_wvsolerr_list[where_c_Kbb]**2))**2)
        c_N_data = np.size(c_good_rv_list[where_c_Kbb])
...
# plt.plot([0,np.size(where_all_c[0])],[1000,1000],linestyle="--",alpha=0.4,color="black",label="Weighted mean uncertainty")
plt.fill_between([0,np.size(where_all_c[0])],rv_star-0.8,rv_star+0.8,alpha=0.3,color="grey",label=r"Stellar RV")
plt.ylim([-20,-5])
for date in np.unique(c_date_list[where_all_c]):
    where_data = np.where(c_date_list[where_all_c]==date)
    first = where_data[0][0]
    last = where_data[0][-1]
    plt.annotate("",xy=(first,-5+rv_star),xytext=(last+len(kl_list)/7.5,-5+rv_star),xycoords="data",arrowprops={'arrowstyle':"|-|",'shrinkA':0.1,'shrinkB':0.1})
    # plt.gca().text((first+last)/2-1,-20+rv_star,date,ha="left",va="top",rotation=-60,size=fontsize)
    plt.gca().text((first+last)/2-1.5,-6+rv_star,date[4:6]+"-"+date[6:8]+"-"+date[0:4],ha="left",va="top",rotation=0,size=fontsize)
plt.gca().text(np.size(where_all_c[0]),-6,"$\kappa$ And b",ha="right",va="top",rotation=0,size=fontsize*1.5)
...
plt.legend(loc="upper left",bbox_to_anchor=[0,1.0],frameon=True,fontsize=fontsize,ncol=3)

# plt.show()
if 1:
    print("Saving "+os.path.join(out_pngs,"kap_And",myoutfilename))
    plt.savefig(os.path.join(out_pngs,"kap_And",myoutfilename),bbox_inches='tight')
    plt.savefig(os.path.join(out_pngs,"kap_And",myoutfilename.replace(".pdf",".png")),bbox_inches='tight')
