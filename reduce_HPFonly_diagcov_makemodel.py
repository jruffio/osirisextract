                    ...
                    if 1:
                        ...
                        if outputdir is not None:
                            ...
                            try:
                                ...
                                hdulist.writeto(os.path.join(outputdir,os.path.basename(filename).replace(".fits","_PSF_transmission.fits")), clobber=True)
                            hdulist.close()
                            hdulist = pyfits.HDUList()
...
if __name__ == "__main__":
    ...
    for filename in filelist:
        ...
        if planet_search:
            ...
            planetRV_array = np.array([hr8799_bary_rv-12.6,])
        ...
        if debug:
            ...
            _process_pixels_onlyHPF(plcen_k_valid_pix[::-1],plcen_l_valid_pix[::-1],row_valid_pix[::-1],col_valid_pix[::-1],
                                    ...
                                    dtype,cutoff,planet_search,(plcen_k,plcen_l),
                                    R_list,
                                    numbasis_list,wvsol_offsets,R_calib_arr,model_persistence=model_persistence,outputdir=outputdir)
            continue
        else:
