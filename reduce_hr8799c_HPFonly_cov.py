...
def _process_pixels_onlyHPF(real_k_indices,real_l_indices,row_indices,col_indices,normalized_psfs_func_list,tlc_spec_list,star_spec,planet_spec_func,wvs,wvshifts_array,dtype,cutoff,planet_search,centroid_guess,iU,U):
    global original,badpix,originalLPF,originalHPF, original_shape, output, output_shape, lambdas, img_center, psfs, psfs_shape, Npixproc, Npixtot
    original_np = _arraytonumpy(original, original_shape,dtype=dtype)
    originalLPF_np = _arraytonumpy(originalLPF, original_shape,dtype=dtype)
    originalHPF_np = _arraytonumpy(originalHPF, original_shape,dtype=dtype)
    ...
    for real_k,real_l,row,col in zip(real_k_indices,real_l_indices,row_indices,col_indices):
        ...
        for wvshift_id in range(nshifts):
            ...
            # if 1:
                ...
                # print(logdet_Sigma,slogdet_icovphi0[1],Npixs_HPFdata,HPFmodel.shape[-1],HPFchi2,np.log(HPFchi2))
...
if __name__ == "__main__":
    ...
    dl_grid,dk_grid = np.meshgrid(np.linspace(-0.5,0.5,50),np.linspace(-0.5,0.5,50))
    ...
    wvshifts_array = np.linspace(-1.5*dwv,-.5*dwv,100)#np.arange(-1.1*dwv,-.8*dwv,dwv/100)
    # wvshifts_array = np.arange(-1*dwv,1*dwv,dwv)#np.arange(-1.1*dwv,-.8*dwv,dwv/100)
    ...
    # for lcorr in [0.25,0.5,0.75,1.0,1.25,1.5]:
    if 1:
