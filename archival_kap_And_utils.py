...
def _tpool_init_detecplanet(mp_cube,mp_noisecube,mp_badpixcube,mp_res,mp_out,mp_cube_model,
                              cube_shape,noisecube_shape,badpixcube_shape, res_shape, out_shape,cube_model_shape):
    """
    ...
    Args:
    """
    global gmp_cube,gmp_noisecube,gmp_badpixcube,gmp_res,gmp_out,gmp_cube_model, \
        gcube_shape,gnoisecube_shape,gbadpixcube_shape, gres_shape, gout_shape,gcube_model_shape
    ...
    gcube_model_shape = cube_model_shape
...
#     wherenans = np.where(np.isnan(myvec_cp))
#     window = int(round(np.size(myvec_cp)/(cutoff/2.)/2.))#cutoff
#     tmp = np.array(pd.DataFrame(myvec_cp).interpolate(method="linear").fillna(method="bfill").fillna(method="ffill"))
#     myvec_cp_lpf = np.array(pd.DataFrame(tmp).rolling(window=window, center=True).median().interpolate(method="linear").fillna(method="bfill").fillna(method="ffill"))[0:np.size(myvec), 0]
#     myvec_cp[wherenans] = myvec_cp_lpf[wherenans]
#     return myvec_cp_lpf,myvec-myvec_cp_lpf

def make_linear_model(cube,noisecube,badpixcube,plx,ply,plrv,center,wvs,transmission,star_spectrum,cube_model,planet_spec_func,science_baryrv,w=0,psfwidth0=0.75,chunks=20,res_model=None,hpf=None):
    nz,ny,nx = cube.shape


    if 0:
        xvec = np.arange(cube.shape[1])
        yvec = np.arange(cube.shape[0])
        zvec = wvs
        # Nl,l,Ns  = 10,1.0,5 # Nw
        Nl,l,Ns  = 6,1,3 # Nw
        yref,xref = int(np.round(center[1]+ply)),int(np.round(center[0]+plx))
        # zref = wvs[-1]
        zref = (wvs[0]+ wvs[-1])/2.
        M_speckles = get_speckle_model(xvec,yvec,zvec,xref,yref,zref,[12+50,35],Nl,l,Ns,w)
        M_speckles = np.reshape(M_speckles,(nz,(2*w+1),(2*w+1),(2*Nl+1)*(2*Nl+1)*Ns))

        M_speckles = np.reshape(M_speckles,(nz*(2*w+1)*(2*w+1),(2*Nl+1)*(2*Nl+1)*Ns))
        u, s, vh = np.linalg.svd(M_speckles, full_matrices=False)
        # print(u.shape, s.shape, vh.shape)
        # print(s)
        # print(np.allclose(M_speckles, np.dot(u * s, vh)))
        # exit()
        new_N_linpara = 24#np.where(s<0.001*np.max(s))[0][0]#(chunks+1)*(2*w+1)**2#1*Ns*(2*w+1)**2
        # print(new_N_linpara,np.size(s))
        # plt.plot(s)
        # plt.show()
        M_speckles = u[:,0:new_N_linpara] * s[0:new_N_linpara]
        # for p in range(new_N_linpara):
        #     plt.plot(M_speckles[:,p],label="{0}".format(p))
        # print(new_N_linpara,(2*Nl+1)*(2*Nl+1)*Ns)
        # plt.legend()
        # plt.show()
        M_speckles = np.reshape(M_speckles,(nz,(2*w+1),(2*w+1),new_N_linpara))


        M_speckles2 = M_speckles*cube_model[:,yref-w:yref+w+1,xref-w:xref+w+1,None]
        M_speckles = M_speckles*star_spectrum[:,None,None,None]
    if 0:
        k,l = int(np.round(center[1]+ply)),int(np.round(center[0]+plx))
        x_knots = wvs[np.linspace(0,nz-1,chunks+1,endpoint=True).astype(np.int)]
        M_spline = get_spline_model(x_knots,wvs,spline_degree=3)
        M_speckles = np.zeros((nz,(2*w+1),(2*w+1),(2*w+1),(2*w+1),(chunks+1)))
        # M_speckles3 = np.zeros((nz,(2*w+1),(2*w+1),(2*w+1),(2*w+1),(chunks+1)))
        for m in range((2*w+1)):
            for n in range((2*w+1)):
                if hpf:
                    # M_speckles[:,m,n,m,n,:]=M_spline*LPFvsHPF((star_spectrum[0]*LPFvsHPF(cube[:,k-w+m,l-w+n],40)[0]/LPFvsHPF(star_spectrum[0],40)[0]))[1][:,None]
                    M_speckles[:,m,n,m,n,:]=M_spline*LPFvsHPF(star_spectrum[0],40)[1][:,None]
                else:
                    M_speckles[:,m,n,m,n,:]=M_spline*star_spectrum[0][:,None]
                    # M_speckles3[:,m,n,m,n,:]=M_spline*star_spectrum[1][:,None]
                    # M_speckles[:,m,n,m,n,:]=M_spline*(star_spectrum*LPFvsHPF(cube[:,k-w+m,l-w+n],40)[0]/LPFvsHPF(star_spectrum,40)[0])[:,None]
                    if 0:
                        _w= np.max([w,1])
                        ww= _w+2
                        tmp_cube_stamp= copy(cube[:,np.max([k-ww,0]):np.nanmin([k+ww+1,ny]),np.max([l-ww,0]):np.nanmin([l+ww+1,nx])])
                        tmp_cube_stamp[:,2:(2+(2*_w+1)),2:(2+(2*_w+1))] = np.nan
                        # plt.imshow(np.nanmean(tmp_cube_stamp,axis=0))
                        # plt.show()
                        tmp_star_spectrum = np.nanmedian(tmp_cube_stamp,axis=(1,2))
                        # plt.plot(tmp_star_spectrum/np.nansum(tmp_star_spectrum),label="tmp_star_spectrum")
                        # plt.plot(star_spectrum/np.nansum(star_spectrum),label="star_spectrum")
                        # plt.legend()
                        # plt.show()
                        M_speckles[:,m,n,m,n,:]=M_spline*tmp_star_spectrum[:,None]
        M_speckles = np.reshape(M_speckles,(nz,(2*w+1),(2*w+1),(chunks+1)*(2*w+1)*(2*w+1)))
        # M_speckles3 = np.reshape(M_speckles3,(nz,(2*w+1),(2*w+1),(chunks+1)*(2*w+1)*(2*w+1)))

        k,l = int(np.round(center[1]+ply)),int(np.round(center[0]+plx))
        x_knots = wvs[np.linspace(0,nz-1,chunks+1,endpoint=True).astype(np.int)]
        M_spline = get_spline_model(x_knots,wvs,spline_degree=3)
        M_speckles2 = np.zeros((nz,(2*w+1),(2*w+1),(2*w+1),(2*w+1),(chunks+1)))
        for m in range((2*w+1)):
            for n in range((2*w+1)):
                if hpf:
                    M_speckles2[:,m,n,m,n,:]=M_spline*LPFvsHPF(cube_model[:,k-w+m,l-w+n],40)[1][:,None]
                else:
                    M_speckles2[:,m,n,m,n,:]=M_spline*cube_model[:,k-w+m,l-w+n,None]
                    # M_speckles2[:,m,n,m,n,:]=M_spline*(cube_model[:,k-w+m,l-w+n]*LPFvsHPF(cube[:,k-w+m,l-w+n],40)[0]/LPFvsHPF(cube_model[:,k-w+m,l-w+n],40)[0])[:,None]
        M_speckles2 = np.reshape(M_speckles2,(nz,(2*w+1),(2*w+1),(chunks+1)*(2*w+1)*(2*w+1)))
    if 0:
        x_knots = wvs[np.linspace(0,nz-1,chunks+1,endpoint=True).astype(np.int)]
        M_spline = get_spline_model(x_knots,wvs,spline_degree=3)
        M_speckles = np.zeros((nz,(2*w+1),(2*w+1),(chunks+1)))
        for m in range((2*w+1)):
            for n in range((2*w+1)):
                M_speckles[:,m,n,:]=M_spline*star_spectrum[0][:,None]
        M_speckles = np.reshape(M_speckles,(nz,(2*w+1),(2*w+1),(chunks+1)))
    if 1:
        x_knots = wvs[np.linspace(0,nz-1,chunks+1,endpoint=True).astype(np.int)]
        M_spline = get_spline_model(x_knots,wvs,spline_degree=3)
        M_speckles = np.zeros((nz,(2*w+1),(2*w+1),(2*w+1),(2*w+1),(chunks+1)))
        for m in range((2*w+1)):
            for n in range((2*w+1)):
                M_speckles[:,m,n,m,n,:]=M_spline*star_spectrum[0][:,None]
        M_speckles = np.reshape(M_speckles,(nz,(2*w+1),(2*w+1),(2*w+1)*(2*w+1)*(chunks+1)))
    if 0 and res_model is not None:
        k,l = int(np.round(center[1]+ply)),int(np.round(center[0]+plx))
        x_knots = wvs[np.linspace(0,nz-1,chunks+1,endpoint=True).astype(np.int)]
        M_spline = get_spline_model(x_knots,wvs,spline_degree=3)
        M_res = np.zeros((nz,(2*w+1),(2*w+1),(2*w+1),(2*w+1),(chunks+1)))
        for m in range((2*w+1)):
            for n in range((2*w+1)):
                M_res[:,m,n,m,n,:]=M_spline*res_model[:,None]
                # M_speckles[:,m,n,m,n,:]=M_spline*cube_model[:,k-w+m,l-w+n,None]
        M_res = np.reshape(M_res,(nz,(2*w+1),(2*w+1),(chunks+1)*(2*w+1)*(2*w+1)))

    if 0:
        N_chunks_bkg = 30
        x_knots = wvs[np.linspace(0,nz-1,N_chunks_bkg+1,endpoint=True).astype(np.int)]
        M_spline = get_spline_model(x_knots,wvs,spline_degree=3)
        # M_background = np.zeros((nz,(2*w+1),(2*w+1),(2*w+1),(2*w+1),(N_chunks_bkg+1)))
        # for m in range((2*w+1)):
        #     for n in range((2*w+1)):
        #         M_background[:,m,n,m,n,:]=M_spline
        # M_background = np.zeros((nz,(2*w+1),(2*w+1),(N_chunks_bkg+1)))
        # M_background = np.reshape(M_background,(nz,(2*w+1),(2*w+1),(2+1)*(2*w+1)*(2*w+1)))
        M_background = np.tile(M_spline[:,None,None,:],(1,(2*w+1),(2*w+1),1))
        M_background2 = np.tile((M_spline*transmission[:,None])[:,None,None,:],(1,(2*w+1),(2*w+1),1))

    # planet_spec = transmission*interpolate.splev(wvs*(1-(plrv-science_baryrv)/const.c.to('km/s').value), science_model_spline, der=0)
    ...
    # x_psf_grid, y_psf_grid = np.meshgrid(np.linspace(-w,w,2*w+1,endpoint=True), np.linspace(-w,w,2*w+1,endpoint=True))
    # for l in range(nz):
    #     nospec_planet_model[l,:,:] = np.exp(-0.5*(x_psf_grid**2+y_psf_grid**2)/(psfwidth0*wvs[l])**2)
    hdfactor = 5
    ...
    # med_wvs = np.median(wvs)
    # for l in range(nz):
    #     nospec_planet_model[l,:,:] = pixgauss2d([1.,w,w,psfwidth0/med_wvs*wvs[l],0.],(2*w+1,2*w+1),xhdgrid=xhdgrid, yhdgrid=yhdgrid)[None,:,:]
    ...
    nospec_planet_model2 =  np.zeros((nz,2*w+1,2*w+1))
    for z in range(nz):
        nospec_planet_model2[z,:,:] = res_model[z](np.arange(-w,w+1,1),np.arange(-w,w+1,1)).transpose()

    # planet_model = nospec_planet_model2*planet_spec[:,None,None]
    # planet_model = nospec_planet_model*planet_spec[:,None,None]

    # plt.subplot(1,3,1)
    # plt.imshow(nospec_planet_model2[50,:,:])
    # plt.subplot(1,3,2)
    # plt.imshow(nospec_planet_model2[1600,:,:])
    # plt.subplot(1,3,3)
    # plt.imshow(nospec_planet_model[1600,:,:])
    # plt.show()

    planet_spec_LPF,planet_spec_HPF = LPFvsHPF(planet_spec,40)
    # planet_model_LPF = nospec_planet_model*planet_spec_LPF[:,None,None]
    # planet_model_HPF = nospec_planet_model2*planet_spec_HPF[:,None,None]
    planet_model = nospec_planet_model2*planet_spec[:,None,None]

    if 1:
        res4model_kl = star_spectrum[1]
        Mkl_list = []
        for k in range(res4model_kl.shape[1]):
            Mkl_list.append((nospec_planet_model[:,:,:, None]*res4model_kl[:,k][:,None,None,None]))

    #LPFvsHPF((star_spectrum[0]*LPFvsHPF(cube[:,k-w+m,l-w+n],40)[0]/LPFvsHPF(star_spectrum[0],40)[0]),40)[1][:,None,None,None]
    k,l = int(np.round(center[1]+ply)),int(np.round(center[0]+plx))
    oldM = np.zeros((nz,2*w+1,2*w+1,2*w+1,2*w+1))
    # oldB = np.zeros((nz,2*w+1,2*w+1,2*w+1,2*w+1))
    for m in range((2*w+1)):
        for n in range((2*w+1)):
            oldM[:,m,n,m,n] = LPFvsHPF((star_spectrum[0]*LPFvsHPF(cube[:,k-w+m,l-w+n],40)[0]/LPFvsHPF(star_spectrum[0],40)[0]),40)[1]
            # oldB[:,m,n,m,n] = np.ones(nz)
            # oldM[:,m,n,m,n] = LPFvsHPF((cube_model[:,k-w+m,l-w+n]*LPFvsHPF(cube[:,k-w+m,l-w+n],40)[0]/LPFvsHPF(cube_model[:,k-w+m,l-w+n],40)[0]),40)[1]

    oldM = np.reshape(oldM,(nz,2*w+1,2*w+1,(2*w+1)*(2*w+1)))
    # oldB = np.reshape(oldB,(nz,2*w+1,2*w+1,(2*w+1)*(2*w+1)))

    #
    HPFmodel = np.concatenate([planet_model[:,:,:, None],M_speckles], axis=3) # best?
    # HPFmodel = np.concatenate([planet_model[:,:,:, None],M_speckles2,M_background2], axis=3)
    # HPFmodel = np.concatenate([planet_model[:,:,:, None],planet_model_LPF[:,:,:, None],M_speckles], axis=3)#
    # HPFmodel = np.concatenate([planet_model[:,:,:, None],planet_model_LPF[:,:,:, None],M_speckles,M_speckles2], axis=3)
    # HPFmodel = np.concatenate([planet_model_HPF[:,:,:, None],planet_model_LPF[:,:,:,None],M_speckles], axis=3)
    # HPFmodel = np.concatenate([planet_model[:,:,:, None],M_speckles]+Mkl_list, axis=3)
    # HPFmodel = np.concatenate([planet_model_HPF[:,:,:, None],oldM], axis=3)
    # HPFmodel = np.concatenate([M_speckles], axis=3)
    # if res_model is not None:
    #     # HPFmodel = np.concatenate([M_speckles2,M_res], axis=3)
    #     # HPFmodel = np.concatenate([planet_model[:,:,:, None],M_speckles2,M_res], axis=3)
    #     HPFmodel = np.concatenate([planet_model[:,:,:, None],M_speckles,M_res], axis=3)
    # else:
    #     HPFmodel = np.concatenate([M_speckles], axis=3)
    # HPFmodel = np.concatenate([M_speckles2,M_speckles], axis=3)
    # HPFmodel = np.concatenate([M_speckles2,M_background2], axis=3)
    # HPFmodel = np.concatenate([M_speckles,M_background2], axis=3)
    # HPFmodel = np.concatenate([M_speckles2], axis=3)
    # HPFmodel = np.concatenate([M_speckles], axis=3)
    # HPFmodel = np.concatenate([M_background,M_speckles], axis=3)
    # HPFmodel = np.concatenate([planet_model[:,:,:, None],M_speckles], axis=3)
    # HPFmodel = M_speckles

    # print(HPFmodel.shape)
    return HPFmodel#,HPFmodel_H0
...
def _task_detecplanet(plx_chunk,ply_chunk,plxid_chunk,plyid_chunk,plrvvec,center,dtype,
                    ...
                    planet_spec_func,science_bary_rv,w,psfwidth0,chunks,res_model):
# def _task_detecplanet(plx_chunk,ply_chunk,plxid_chunk,plyid_chunk,plrvvec,center,dtype):
#     print(plx_chunk,ply_chunk,plxid_chunk,plyid_chunk)
    global gmp_cube,gmp_noisecube,gmp_badpixcube,gmp_res,gmp_out,gmp_cube_model, \
        gcube_shape,gnoisecube_shape,gbadpixcube_shape, gres_shape, gout_shape,gcube_model_shape
    ...
    cube_model_np = _arraytonumpy(gmp_cube_model, gcube_model_shape,dtype=dtype)
    ...
    for rvid,rv in enumerate(plrvvec):
        for plx,ply,plxid,plyid in zip(plx_chunk,ply_chunk,plxid_chunk,plyid_chunk):
            ...
            M = make_linear_model(cube_np,noisecube_np,badpixcube_np,plx,ply,rv,center,
                          wvs,telluric_transmission,star_spectrum,cube_model_np,planet_spec_func,science_bary_rv,w,psfwidth0,chunks,res_model,hpf)
            ...
            if hpf:
                ...
                stamp_cube_ravel = np.ravel(cube_np[:,k-w:k+w+1,l-w:l+w+1])
            ...
            if np.size(where_finite[0]) <= 0.25*(nz_stamp*ny_stamp*nx_stamp):
                continue
            ...
            # if 1:
                ...
                # out_shape = [(1+2*N_linpara),np.size(plrvvec),plx_grid.shape[0],plx_grid.shape[1]]
                # dim 1: likelihood + best lin para values + uncertainties for lin para values
                # dim 2: RVs
                # dim 3: locations
                # print((N_data-1+N_linpara-1)/2)
                # print((N_data-1+N_linpara-1-1)/2)
                # print(loggamma((N_data-1+N_linpara-1)/2))
                # print(loggamma((N_data-1+N_linpara-1-1)/2))
                # print((N_linpara-N_data)/2*np.log(2*np.pi))
                # print(((N_linpara-1)-N_data)/2*np.log(2*np.pi))
                ...
                if 0:# and res_model is not None:
                    print(paras)
                    print(paras/np.sqrt(np.diag(covphi)))
                    print(out_np[0,rvid,plyid,plxid],out_np[1,rvid,plyid,plxid])
                    plt.figure(1)
                    plt.subplot(2,1,1)
                    plt.plot(data,label="data")
                    plt.plot(model,label="model (planet + speckle)")
                    # plt.plot(model_H0,label="model H0")
                    plt.plot(paras[0]*M_ravel[:,0]-1,label="planet model")
                    # plt.plot(paras[1]*M_ravel[:,1],label="planet model LPF")
                    plt.plot(model-paras[0]*M_ravel[:,0]-1,label="speckle model")#
                    # plt.plot(paras[-1]*M_ravel[:,-1],label="planet model")
                    # plt.plot(model-paras[-1]*M_ravel[:,-1],label="speckle model")
                    plt.plot(residuals-2,label="Residuals")
                    # plt.plot(sigmas,label="sigmas")
                    # plt.plot(np.sqrt(chi2 / N_data)*sigmas,label="chi2 / N_data*sigmas")
                    print(chi2,chi2_H0,np.size(paras))
                    print(data[0:10],data[-1])
                    print(np.size(np.where(np.abs(residuals)<(np.sqrt(chi2 / N_data)*sigmas))[0])/N_data)
                    plt.legend()

                    plt.subplot(2,1,2)
                    # plt.plot(np.cumsum((residuals/sigmas) ** 2),label="Residuals")
                    # plt.plot(np.cumsum((residuals_H0/sigmas) ** 2),label="Residuals H0")
                    plt.plot(np.cumsum((residuals_H0/sigmas) ** 2-(residuals/sigmas) ** 2),label="Residuals H0 - H1")
                    plt.legend()
                    ...
                    plt.plot(np.arange(-100,101),res_ccf[(res_ccf_argmax-100):(res_ccf_argmax+101)]/res_ccf[res_ccf_argmax],label="H1")

                    residuals_H0[np.where(np.isnan(residuals_H0))] = 0
                    # _res = np.ones(_res.shape)
                    res_ccf = np.correlate(residuals_H0,residuals_H0,mode="same")/np.size(residuals_H0)
                    res_ccf_argmax = np.nanargmax(res_ccf)
                    plt.plot(np.arange(-100,101),res_ccf[(res_ccf_argmax-100):(res_ccf_argmax+101)]/res_ccf[res_ccf_argmax],label="H0")

                    residuals_c = np.ravel(canvas)
                    residuals_c[np.where(np.isnan(residuals_c))] = 0
                    # _res = np.ones(_res.shape)
                    res_ccf = np.correlate(residuals_c,residuals_c,mode="same")/np.size(residuals_c)
                    res_ccf_argmax = np.nanargmax(res_ccf)
                    plt.plot(np.arange(-100,101),res_ccf[(res_ccf_argmax-100):(res_ccf_argmax+101)]/res_ccf[res_ccf_argmax],label="canvas")

                    plt.legend()
                    plt.ylim([-0.05,0.5])

                    plt.figure(3)
                    res_hist,bin_edges = np.histogram(residuals/(sigmas*np.sqrt(chi2 / N_data)),bins=100,range=[-20,20])
                    bin_center = (bin_edges[1::]+bin_edges[0:np.size(bin_edges)-1])/2
                    plt.plot(bin_center,res_hist/np.max(res_hist),label="res")
                    plt.plot(np.linspace(-7,7,200),np.exp(-0.5*np.linspace(-7,7,200)**2),linestyle="--",linewidth=1,color="black",label="Gaussian")#1/np.sqrt(2*np.pi)*
                    plt.yscale("log")
                    plt.legend()
                    ...
                    for m in range((2*w+1)):
                        for n in range((2*w+1)):
                            plt.plot(canvas[:,m,n],color="grey")
                    plt.plot(res_np[:,k,l],linewidth = 3,color="orange")
...
def detecplanet(cube,w,center=None,plxvec=None,plyvec=None,plrvvec=None, noisecube=None, badpixcube=None,numthreads=16,dtype= ctypes.c_double,
                      wvs=None,telluric_transmission=None,star_spectrum=None,cube_model =None,
                    planet_spec_func=None,science_bary_rv=None,psfwidth0=None,chunks=None,res_model=None):
    ...
    if cube_model is not None:
        cube_model_pad = np.pad(cube_model,((0,0),(w,w),(w,w)),mode="constant",constant_values=np.nan)
    else:
        cube_model_pad = copy(cube_pad)

    M = make_linear_model(cube_pad,noisecube_pad,badpixcube_pad,plxvec[0],plyvec[0],plrvvec[0],center_pad,
                          wvs,telluric_transmission,star_spectrum,cube_model,planet_spec_func,science_bary_rv,w,psfwidth0,chunks,res_model)
    nz_stamp,ny_stamp,nx_stamp,N_linpara = M.shape
    ...
    mp_cube_model = mp.Array(dtype, np.size(cube_model_pad))
    cube_model_shape = cube_model_pad.shape
    cube_model_np = _arraytonumpy(mp_cube_model, cube_model_shape,dtype=dtype)
    cube_model_np[:] = copy(cube_model_pad)

    if numthreads == 0:
        _tpool_init_detecplanet(mp_cube,mp_noisecube,mp_badpixcube,mp_res,mp_out,mp_cube_model,
                                  cube_shape,noisecube_shape,badpixcube_shape, res_shape,out_shape,cube_model_shape)
        _task_detecplanet(plx_grid_ravel,ply_grid_ravel,plxid_grid_ravel,plyid_grid_ravel,plrvvec,center_pad,dtype,
                          wvs,telluric_transmission,star_spectrum,planet_spec_func,science_bary_rv,w,psfwidth0,chunks,res_model)
    else:
        ...
        tpool = mp.Pool(processes=numthreads, initializer=_tpool_init_detecplanet,
                        initargs=(mp_cube,mp_noisecube,mp_badpixcube,mp_res,mp_out,mp_cube_model,
                                  cube_shape,noisecube_shape,badpixcube_shape, res_shape,out_shape,cube_model_shape),
                        maxtasksperchild=50)
