...
if __name__ == "__main__":
    ...
    if 0: # remove outliers:
        ...
        refopti = [fitCtoO_list[myargmax[2]],fitT_list[myargmax[0]],fitlogg_list[myargmax[1]]]
        ...
        for file_id, filename in enumerate(new_file_list):
            ...
            myargmax = np.unravel_index(np.nanargmax(tmp_combinedpost_rvmargi),tmp_combinedpost_rvmargi.shape)
            # print(myargmax)
            print(fitCtoO_list[myargmax[2]]-refopti[2],fitT_list[myargmax[0]]-refopti[0],fitlogg_list[myargmax[1]]-refopti[1],filename)
        exit()
...
# 0.0 0.0 -0.033333333333333215 /data/osiris_data/HR_8799_b/20161108/reduced_jb/s161108_a043002_Kbb_020.fits
    ...
    print(fitCtoO_list)
    print(date_list)
    plt.ylim([fitCtoO_list[0]-0.1,fitCtoO_list[-1]])
    ...
    for date in unique_dates:
        ...
        plt.gca().text((first+last)/2-0.5,fitCtoO_list[0]-0.01,date[2:4]+"-"+date[4:6]+"-20"+date[0:2],ha="left",va="top",rotation=-60,size=fontsize)
    ...
    hdulist.append(pyfits.ImageHDU(data=hr_fitCtoO_list))
    ...
    final_leftCI,final_yval,final_rightCI,_ = get_err_from_posterior(hr_fitCtoO_list,np.nansum(combined_post,axis=(0,1,3)))
    ...
    print(myargmax)
    print(hr_fitT_list[myargmax[0]],hr_fitlogg_list[myargmax[1]],hr_fitCtoO_list[myargmax[2]])
    for k in range(Nparas):
        dims = np.arange(Nparas).tolist()
        dims.pop(k)
