...
if 1:
    ...
    where_esti_c = np.where(np.isfinite(c_good_rv_list)*(c_mjdobs_list > 55388)*(c_mjdobs_list < 55767))
    ...
    where_c_Kbb = np.where(np.isfinite(c_good_rv_list[where_all_c])*(c_mjdobs_list[where_all_c] < 55767)*(np.abs(c_rel_res[where_all_c])<3)*("Kbb"==c_ifs_fitler_list[where_all_c]))
    where_c_Hbb = np.where(np.isfinite(c_good_rv_list[where_all_c])*(c_mjdobs_list[where_all_c] < 55767)*(np.abs(c_rel_res[where_all_c])<3)*("Hbb"==c_ifs_fitler_list[where_all_c]))
    where_c_Kbbbad = np.where(np.isfinite(c_good_rv_list[where_all_c])*((c_mjdobs_list[where_all_c] >= 55767)+(np.abs(c_rel_res[where_all_c])>=3))*("Kbb"==c_ifs_fitler_list[where_all_c]))
    where_c_Hbbbad = np.where(np.isfinite(c_good_rv_list[where_all_c])*((c_mjdobs_list[where_all_c] >= 55767)+(np.abs(c_rel_res[where_all_c])>=3))*("Hbb"==c_ifs_fitler_list[where_all_c]))
    ...
    where_esti_c = np.where(np.isfinite(c_good_rv_list)*(c_mjdobs_list > 55388)*(c_mjdobs_list < 55767)*(np.abs(c_rel_res)<3))
    ...
    c_uniquedate_list = []
    for k,day in enumerate(unique_date):
        where_c = np.where(np.isfinite(c_good_rv_list)*(c_mjdobs_list > 55388)*(c_mjdobs_list < 55767)*(day == c_int_mjdobs_list)*(np.abs(c_rel_res)<3))
    ...
    if 1:
        ...
        b_combined_avg,b_combined_sig,_ = get_err_from_posterior(final_planetRV_hd,b_pessimistic_posterior)
        c_combined_avg,c_combined_sig,_ = get_err_from_posterior(final_planetRV_hd,c_pessimistic_posterior)
    ...
    plt.errorbar(b_combined_avg_list[np.where(b_N_data_list>3)],np.arange(np.size(unique_date))[np.where(b_N_data_list>3)],xerr=b_combined_sig_list[np.where(b_N_data_list>3)],fmt="none",color="#0099cc")
    plt.plot(b_combined_avg_list[np.where(b_N_data_list>3)],np.arange(np.size(unique_date))[np.where(b_N_data_list>3)],"x",color="#0099cc",label="b (# exposures)")
    eb1 = plt.errorbar(b_combined_avg_list[np.where(b_N_data_list<=3)],np.arange(np.size(unique_date))[np.where(b_N_data_list<=3)],xerr=b_combined_sig_list[np.where(b_N_data_list<=3)],fmt="none",color="#0099cc")
    eb1[-1][0].set_linestyle("--")
    plt.plot(b_combined_avg_list[np.where(b_N_data_list<=3)],np.arange(np.size(unique_date))[np.where(b_N_data_list<=3)],"x",color="#0099cc")
    ...
    plt.gca().spines["left"].set_position(("data",-15))
    ...
    plt.plot([c_combined_avg,c_combined_avg],[0,10],linestyle="-",linewidth=2,color="#cc3300",label="c: RV estimate")
    plt.gca().text(c_combined_avg,0.1,"${0:.1f}\pm {1:.1f}$ km/s".format(c_combined_avg,c_combined_sig),ha="right",va="bottom",rotation=90,size=fontsize,color="#cc3300")
    ...
    plt.fill_between(final_planetRV_hd,c_posterior,c_pessimistic_posterior,linestyle="-",linewidth=3,color="#ff9900",alpha=0.1,label="c: posterior uncertainty")
    ...
    plt.plot([b_combined_avg,b_combined_avg],[0,10],linestyle="-.",linewidth=2,color="#003366",label="b: RV estimate")
    plt.gca().text(b_combined_avg,0.1,"${0:.1f}\pm {1:.1f}$ km/s".format(b_combined_avg,b_combined_sig),ha="right",va="bottom",rotation=90,size=fontsize,color="#003366")
    ...
    plt.fill_between(final_planetRV_hd,b_posterior,b_pessimistic_posterior,linestyle="-",linewidth=3,color="#0099cc",alpha=0.1,label="b: posterior uncertainty")
    ...
    deltaRV,deltaRV_sig,_ = get_err_from_posterior(final_planetRV_hd,delta_pessimistic_posterior)
    ...
    plt.gca().text(deltaRV,0.1,"${0:.1f}\pm {1:.1f}$ km/s".format(deltaRV,deltaRV_sig),ha="right",va="bottom",rotation=90,size=fontsize,color="#660066")
    ...
    plt.fill_between(final_planetRV_hd,delta_posterior,delta_pessimistic_posterior,linestyle="-",linewidth=3,color="#6600ff",alpha=0.1,label="posterior uncertainty") #9966ff
