...
for fakecorr in [False]:#[False,True]:
    ...
    for nbid,numbasis in enumerate(numbasis_list):
        myoutfilename = "RV_HR_8799_b_measurements_kl{0}.pdf".format(numbasis)
        if numbasis == 0:
            mylabel = "No PCA"
            mymark = "x"
            mycolor = "#ff9900"
        ...
        if numbasis ==0:
            c_fileinfos_filename = "/data/osiris_data/HR_8799_b/fileinfos_Kbb_jb.csv"
        else:
            c_fileinfos_filename = "/data/osiris_data/HR_8799_b/fileinfos_Kbb_jb_kl{0}.csv".format(numbasis)
        ...
        if 1:
            ...
            if fakecorr:
                ...
                c_rvoffsets_list = -9.06-c_rvfakes_list
            ...
            where_esti_c = np.where(np.isfinite(c_good_rv_list)*(c_mjdobs_list > 55388))
            ...
            if 1:
                ...
                c_N_data = np.sum(c_itime_list[where_esti_c])
                ...
                try:
                    hdulist.writeto(os.path.join(out_pngs,"HR_8799_b",myoutfilename.replace(".pdf","_posterior.fits")), overwrite=True)
                except TypeError:
                    hdulist.writeto(os.path.join(out_pngs,"HR_8799_b",myoutfilename.replace(".pdf","_posterior.fits")), clobber=True)
                hdulist.close()
            ...
            if 1: #print estimates per night
                ...
                for k,day in enumerate(unique_date):
                    ...
                    # print(c_yr_list_new)
                    # print((day == np.array(c_yr_list_new)))
                ...
                print(numbasis)
                # print(np.array(unique_date))
                # print(np.array(c_combined_avg_list))
                # print(np.array(c_combined_sig_list))
                # print(np.array(c_N_data_list))
                # print(np.array(c_mjd_list))
                # exit()
    ...
    # plt.show()
    if 1:
        print("Saving "+os.path.join(out_pngs,"HR_8799_b",myoutfilename))
        plt.savefig(os.path.join(out_pngs,"HR_8799_b",myoutfilename),bbox_inches='tight')
        plt.savefig(os.path.join(out_pngs,"HR_8799_b",myoutfilename.replace(".pdf",".png")),bbox_inches='tight')
