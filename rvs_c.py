...
for fakecorr in [False]:#[False,True]:
    ...
    for nbid,numbasis in enumerate(numbasis_list):
        myoutfilename = "RV_HR_8799_c_measurements_kl{0}.pdf".format(numbasis)
        if numbasis == 0:
            mylabel = "No PCA"
            mymark = "x"
            mycolor = "#ff9900"
        ...
        if 1:
            ...
            if fakecorr:
                ...
                c_rvoffsets_list = -11.13-c_rvfakes_list
            ...
            if 0:
                ...
                for date in np.unique(c_date_list):
                    ...
                    for k in np.arange(first,last+1,d):
                        ...
                        c_mjdobs_list_new.append(np.mean(c_mjdobs_list[k:k+d]))
                    # exit()
                ...
                c_mjdobs_list = np.array(c_mjdobs_list_new)
            ...
            where_all_c = np.where(np.isfinite(c_good_rv_list)*(c_mjdobs_list > 55388))#*(c_mjdobs_list < 56498))
            ...
            if 1:
                ...
                try:
                    hdulist.writeto(os.path.join(out_pngs,"HR_8799_c",myoutfilename.replace(".pdf","_posterior.fits")), overwrite=True)
                except TypeError:
                    hdulist.writeto(os.path.join(out_pngs,"HR_8799_c",myoutfilename.replace(".pdf","_posterior.fits")), clobber=True)
                hdulist.close()
    ...
    plt.plot([0,np.size(where_all_c[0])],[100,100],color="black",label="Weighted Mean")
    ...
    for date in np.unique(c_date_list[where_all_c]):
        ...
        plt.gca().text((first+last)/2-1,-17+rv_star,date,ha="left",va="top",rotation=-60,size=fontsize)
    ...
    # plt.show()
    if 1:
        print("Saving "+os.path.join(out_pngs,"HR_8799_c",myoutfilename))
        plt.savefig(os.path.join(out_pngs,"HR_8799_c",myoutfilename),bbox_inches='tight')
        plt.savefig(os.path.join(out_pngs,"HR_8799_c",myoutfilename.replace(".pdf",".png")),bbox_inches='tight')
    ...
    if fakecorr:
        ...
        plt.errorbar(numbasis_list,combined_RV_list,yerr=combined_RVerr_list,label="Corrected from simulated planets")
    else:
        plt.plot(numbasis_list,combined_RV_list)
